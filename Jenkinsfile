...
pipeline {
  ...
  stages {
    ...
    // Compute a new semantic version from latest tag vX.Y.Z (fallback 1.0.0)
    stage('Set Version') {
      steps {
        script {
          def lastTag = sh(script: "git tag --list 'v*.*.*' --sort=-v:refname | head -n 1", returnStdout: true).trim()
    ...
    // Build the Docker image from your Dockerfile (which runs `npm ci` + `npm run build`)
    stage('Docker: Build & Push') {
      steps {
        script {
          def buildArgs = "--pull --no-cache ." // first clean build to avoid old layers
          docker.withRegistry('https://index.docker.io/v1/', DOCKERHUB_CREDS) {
            def img = docker.build("${DOCKERHUB_REPO}:${env.IMAGE_TAG}", buildArgs)
            img.push() // :X.Y.Z
            // Push :latest only on main
            def branch = env.BRANCH_NAME ?: sh(script: 'git rev-parse --abbrev-ref HEAD', returnStdout: true).trim()
  ...
  post {
    success {
      echo "✅ Pushed ${DOCKERHUB_REPO}:${env.IMAGE_TAG}${(env.BRANCH_NAME ?: 'main') == 'main' ? ' and :latest' : ''}"
    }
    cleanup {
      cleanWs()
    }
